...
target_items = [
    ...
    "TESTING & COMMISSIOINING OF THE POOL", # Typo from PDF
    "VACCUM HEAD", # Typo from PDF
    "VACCUM HOSE", # Typo from PDF
    "TELESCOPIC ROD",
    "DEEP LEAF RAKE",
    "WATER TEST KIT",
    "POOL BRUSH",
...
def clean_name(name):
    # Map back to correct spelling for filenames
...
for page_data in structure:
    ...
    for item in target_items:
        # Find the text block for this item
        item_bbox = None
        ...
        if item_bbox:
            ...
            if nearest_img and min_dist < 500:
                img_name = clean_name(item) + ".jpg"
                out_path = os.path.join("public/template-images/mr-swimming-pools", img_name)
                
                # Extract image
                pix = page.get_pixmap(clip=nearest_img["bbox"])
                pix.save(out_path)
                
                # Use clean item name in mapping
                clean_item_key = item.replace("VACCUM", "VACUUM").replace("COMMISSIOINING", "COMMISSIONING")
                mapping[clean_item_key] = img_name
                print(f"Mapped '{item}' to {img_name}")

doc.close()

with open("scratch/final_image_mapping.json", "w") as f:
    json.dump(mapping, f, indent=2)

print(f"Extraction complete. {len(mapping)} images mapped.")
